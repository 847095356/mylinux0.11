...
int sys_kill(int pid, int sig)
{
	...
	if (!pid)
		...
		while (--p > &FIRST_TASK) // pid=-1给任何进程发送
			if (err = send_sig(sig, *p, 0))
				retval = err;
			else
				while (--p > &FIRST_TASK) // pid<-1 给进程组发送信息
					if (*p && (*p)->pgrp == -pid)
						if (err = send_sig(sig, *p, 0))
							retval = err;
	return retval;
...
//告诉父进程要死了 通知被销毁进程的父进程：通过遍历task结构体数组找到父进程的task结构体，然后增加signal字段的sigchld值！
...
//父进程创建子进程时，有时候需要等待子进程执行完毕，需要调用sys_waitpid函数阻塞父进程自己，
//如下：如果子进程是僵死状态，就把子进程运行的时间叠加到父进程，然后释放子进程的task结构体！
//如果子进程还在运行，父进程通过schedule让出cpu，把自己阻塞；
//下次被唤醒后再次检查子进程是否给自己发送了SIGCHLD信号；
//如果没收到，重复检查的过程，直到子进程庄涛变为僵死后返回继续执行后续的代码！
